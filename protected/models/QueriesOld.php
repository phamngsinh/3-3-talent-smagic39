...
class Queries extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
		    ...
		    array('salutation', 'required','message'=>"Please choose salutation"),
			
			array('name, contact_number, email_address, subject, comments', 'required'),
			
			array('email_address', 'email'),
		    
			array('is_read', 'numerical', 'integerOnly'=>true),
			array('salutation, ip_address', 'length', 'max'=>20),
			array('name, email_address', 'length', 'max'=>200),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, salutation,contact_number, name, email_address, subject, comments, date, ip_address, is_read', 'safe', 'on'=>'search'),
