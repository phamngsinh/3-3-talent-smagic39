...
class Watermarker extends CController
{
    ...
    var $txt_watermark_font_size = 20;
    
    var $txt_watermark_font_style = 'fonts/MyriadPro-Regular.otf';
    
    var $txt_watermark_font_angle = 0;
    
    var $txt_watermark_font_y = 10;
	...
	$this->img["path"] =  $imgfile;  //original dimensions
	...
	if ($gd_version >= 2)
	{
	    ...
	    if ($this->txt_watermark != '')
	    {
		//this line is commented by shahid
		...
		/* code added by shahid **/

		$font = 'fonts/MyriadPro-Regular.otf';
		
		//center it
		$dimensions = imagettfbbox($this->txt_watermark_font_size, $this->txt_watermark_font_angle, $this->txt_watermark_font_style, $this->txt_watermark);
		$textWidth = abs($dimensions[4] - $dimensions[0]);
		$x = (imagesx($this->img["des"]) - $textWidth)/2;

		if($this->img_watermark_Valing == "BOTTOM")
		{
		    		    
		    $this->txt_watermark_Hmargin = imagesy($this->img["des"])-62;
		}
		
		

		// Add the text
		imagettftext($this->img["des"], $this->txt_watermark_font_size, $this->txt_watermark_font_angle, $this->txt_watermark_Vmargin, $this->txt_watermark_Hmargin, $txt_color, $this->txt_watermark_font_style, $this->txt_watermark);
    ...
    function processToAdd($position)
    {
	...
	{
	    $this->img["watermark"] = ImageCreateFromJPEG($this->img_watermark);
	}
	...
	if ($gd_version >= 2)
	{
	    ...
	    $this->img["des"] = ImageCreateTrueColor($X_des, $Y_des+$this->img["y_watermark"]);
		...
		imagecopy($this->img["des"],$this->img["src"],0,$this->img["y_watermark"],0,0,540,$this->img["y"]);
		...
		// Add the text
		imagettftext($this->img["des"], $this->txt_watermark_font_size, $this->txt_watermark_font_angle, $x, $this->txt_watermark_Hmargin, $txt_color, $this->txt_watermark_font_style, $this->txt_watermark);
	...
	//$W_mark =  $this->txt_watermark_font_size * strlen($this->txt_watermark);
...
// End gdVersion()
    ...
    /**
     * NOT USED : to do: mezclar imagenes a tama�o original, preservar canal alpha y redimensionar
